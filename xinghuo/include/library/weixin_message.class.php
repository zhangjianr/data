...
class weixin_message{
	...
	public function sendordernotice(){
		...
		foreach($lists as $v){
			$this->sendorder_topay($v["Order_ID"],$v["User_ID"]);
	...
	public function sendorder_topay($orderid,$userid){
		$u0 = $this->db->GetRs("user","Owner_Id,User_NickName","where Users_ID='".$this->usersid."' and User_ID=".$userid);
		$name = $u0["User_NickName"];
		$u1 = $this->getownerinfo($u0["Owner_Id"]);
		$b1 = $b2 = 1;
		if(is_array($u1)){
			$bonus = $this->get_bonus($orderid, $u0["Owner_Id"]);
			$text = '您推荐的一级会员'.$name.'有未付款订单，您即将获取佣金'.$bonus.'元，请提醒他付款';
			$this->sendmessage($u1["User_OpenID"],$text);
			$u2 = $this->getownerinfo($u1["Owner_Id"]);
			if(is_array($u2)){
				$bonus = $this->get_bonus($orderid, $u1["Owner_Id"]);
				$text = "您推荐的二级会员".$name."有未付款订单，您即将获取佣金".$bonus."元，请提醒他付款";
				$this->sendmessage($u2["User_OpenID"],$text);				
				$u3 = $this->getownerinfo($u2["Owner_Id"]);
				if(is_array($u3)){
					$bonus = $this->get_bonus($orderid, $u2["Owner_Id"]);
					$text = '您推荐的三级会员'.$name.'有未付款订单，您即将获取佣金'.$bonus.'元，请提醒他付款';
				}
			}
		}
	}
