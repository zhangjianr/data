...
var user_obj={
	...
	user_init:function(){
		...
		$('.upd_money').dblclick(function(){
			...
			var s_html='<select><option value="0">加余额</option></select><br /><input value="" />';
			o.data('text', o.html()).html(s_html);
			o.children('input').select();
			o.children('input, select').keyup(function(event){
				if(event.which==13){
	...
	gift_orders_init:function(){
		...
		$('#virtual_form input:submit').click(function(){
			if(global_obj.check_form($('*[notnull]'))){return false};
			$(this).attr('disabled', true);
			window.self.location='gift_orders_virtual.php?code='+$('#Code').val();
		});
		...
		$('#recieve_form input:submit').click(function(){
			window.self.location='gift_orders_recieve.php';
		});
	},
}