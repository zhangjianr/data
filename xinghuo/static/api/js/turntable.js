var turntable_obj={
	turntable_init:function(){
		...
		$("#startbtn").rotate({
			bind:{
				click:function(){
					...
					$.post(ajax_url,'action=move', function(data){
		...
		$('#GetPrize input:submit').click(function(){
			...
			$.post(ajax_url, $('#GetPrize').serialize()+'&action=mobile', function(data){
				if(data.status==1){
	...
	use_sn_init:function(){
		...
		$('#UsedPrize input:submit').click(function(){
			...
			$(this).attr('disabled', true).val('提交中...');
			$.post(ajax_url, $('#UsedPrize').serialize()+'&action=used', function(data){
				if(data.status==1){
...
$(document).ready(function(){
    ...
    $('#jifen').click(function(){
	      $.post(ajax_url, 'action_do=jifen', function(data){
				if(data.status==1){
