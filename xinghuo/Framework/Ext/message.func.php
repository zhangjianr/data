...
function user_order_tem($orderid,$id,$url){
	...
	if($r){
		...
		if(empty($r["Order_Shipping"])){
			...
			$Shippings = json_decode($r["Order_Shipping"],true);
		...
		$r["Order_Qty"] = 0;
		$cartlist = '';
		$CartList=json_decode($r["Order_CartList"],true);
		if($id==3){
 ...
 function user_integral_message_tem($usersid,$userid,$val,$url=''){
	$url = "http://distribute.wybao.cn/api/".$usersid."/user/gift/?wxref=mp.weixin.qq.com";
	global $DB1;
	$data = array();
	$mess = '';
	$access_token = get_token_tem($usersid);
