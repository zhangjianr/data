...
var shop_obj = {
    page_init: function() {		
        ...
        $("#confirm_receive").click(function() {
		   ...
		   $("#confirm_receive").attr("disabled", "true");
            ...
            $.post(base_url + 'api/' + UsersID + '/shop/member/ajax/', param, function(data) {
				...
				if(data.status == 1){
				   ...
				   // alert (data.msg);
					...
					//$("#confirm_receive").removeAttribute("href"); 
				}else if(data.status ==2){
				    $("#confirm_receive").val("请勿重复提交!");
					$("#confirm_receive").css("cssText","background-color:gray!important;display:block!important;height:36px!important;margin:0 auto!important;width:150px!important");
        ...
        $('footer').on('click','#menu-addtocard-btn,#menu-direct-btn"',function(){
			...
			$("#option_content").html('');	
			$("footer").html(selector_content);
			var footer_height = window_half_height+90;
            $("footer").height(footer_height);
                ...
                if (data.status == 1) {
					if($("#buy-type").html() == "menu-direct-btn"){
						if(is_virtual==1){
							...
							window.location.href = "/api/"+UsersID+"/shop/cart/checkout/"+needcart+"/";
						}
					}
					//购物车特效
				    $.cookie(UsersID+'car_num', data.qty, {expires:7,path:'/'});
			...
			$.post(url,{is_distribute:is_distribute,productid:productid,action:'distribute_product'},function(data){
				 if (data.status == 0) {
                       ...
                       if(data.Is_Distribute == 1){
					   		window.location.href = "/api/"+UsersID+'/shop/distribute/distribute_goods/'+productid+'/';
					   }else{
					   		window.location.href = "/api/"+UsersID+'/shop/distribute/join/';
					   }
                   }
