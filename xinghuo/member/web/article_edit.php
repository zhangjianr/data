...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div id="column" class="r_con_wrap">
      ...
      <form class="r_con_form" method="post" action="article_edit.php">
          ...
          <select name='LinkUrl'>
            ...
            <?php $DB->getPage("wechat_material","Material_ID,Material_Table,Material_Json","where Users_ID='".$_SESSION["Users_ID"]."' and Material_Table<>'0' and Material_TableID=0 and Material_Display=0 order by Material_ID desc");
				while($Material=$DB->fetch_assoc()){
					...
					echo '<option value="/api/'.$_SESSION["Users_ID"].'/'.$Material['Material_Table'].'/"'.($rsArticle["Article_LinkUrl"]=='/api/'.$_SESSION["Users_ID"].'/'.$Material['Material_Table'].'/'?" selected":"").'>'.$Material_Json['Title'].'</option>';
				}?>
            ...
            <?php $DB->get("web_column","*","where Users_ID='".$_SESSION["Users_ID"]."' order by Column_Index asc");
				...
				foreach($ColumnList as $key=>$value){
					echo '<option value="/api/'.$_SESSION["Users_ID"].'/web/column/'.$value['Column_ID'].'/"'.($rsArticle["Article_LinkUrl"]=='/api/'.$_SESSION["Users_ID"].'/web/column/'.$value['Column_ID'].'/'?" selected":"").'>'.$value['Column_Name'].'</option>';
					$result = mysql_query("select * from web_article where Users_ID='".$_SESSION["Users_ID"]."' and Column_ID=".$value['Column_ID']);
					while($Article = mysql_fetch_assoc($result)){
						echo '<option value="/api/'.$_SESSION["Users_ID"].'/web/article/'.$Article["Article_ID"].'/"'.($rsArticle["Article_LinkUrl"]=='/api/'.$_SESSION["Users_ID"].'/web/article/'.$Article['Article_ID'].'/'?" selected":"").'>&nbsp;&nbsp;├'.$Article["Article_Title"].'</option>';
					}
				}?>
            <option value="/api/<?php echo $_SESSION["Users_ID"] ?>/web/lbs/"<?php echo $rsArticle["Article_LinkUrl"]=='/api/'.$_SESSION["Users_ID"].'/web/lbs/'?" selected":"" ?>>一键导航(LBS)</option>
            ...
            foreach($ParentCategory as $key=>$value){
                ...
                if($DB->num_rows()>0){
                    echo '<option value="/api/'.$_SESSION["Users_ID"].'/shop/category/'.$value["Category_ID"].'/" '.($rsArticle["Article_LinkUrl"]=='/api/'.$_SESSION["Users_ID"].'/shop/category/'.$value["Category_ID"].'/'?" selected":"").'>'.$value["Category_Name"].'</option>';
                    while($rsCategory=$DB->fetch_assoc()){
                        echo '<option value="/api/'.$_SESSION["Users_ID"].'/shop/category/'.$rsCategory["Category_ID"].'/" '.($rsArticle["Article_LinkUrl"]=='/api/'.$_SESSION["Users_ID"].'/shop/category/'.$rsCategory["Category_ID"].'/'?" selected":"").'>&nbsp;&nbsp;├'.$rsCategory["Category_Name"].'</option>';
                    }
                }else{
                    echo '<option value="/api/'.$_SESSION["Users_ID"].'/shop/category/'.$value["Category_ID"].'/" '.($rsArticle["Article_LinkUrl"]=='/api/'.$_SESSION["Users_ID"].'/shop/category/'.$value["Category_ID"].'/'?" selected":"").'>'.$value["Category_Name"].'</option>';
                }
            ...
            <?php $DB->get("wechat_url","*","where Users_ID='".$_SESSION["Users_ID"]."'");
				  while($Url=$DB->fetch_assoc()){
					  echo '<option value="'.$Url['Url_Value'].'"'.($rsArticle["Article_LinkUrl"]==$Url["Url_Value"]?" selected":"").'>'.$Url['Url_Name'].'('.$Url['Url_Value'].')</option>';
				  }?>
          </select>
          </span> </span>
          <div class="clear"></div>
