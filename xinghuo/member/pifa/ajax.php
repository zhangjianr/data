...
$template_dir = $_SERVER["DOCUMENT_ROOT"].'/member/pifa/lbi/';
...
if($action == 'get_product'){
   ...
   if(strlen($cate_id)>0){
	    	//获取分类列表
	    $rsCategory=$DB->GetRs("pifa_category","*","where Users_ID='".$UsersID."' and Category_ID=".$cate_id);
		if(empty($rsCategory["Category_ParentID"])){
			$CategoryList=array();
			$DB->Get("pifa_category","*","where Users_ID='".$UsersID."' and Category_ParentID=".$cate_id);
   ...
   $rsProducts = $DB->Get("pifa_products",'Products_ID,Products_Name,Products_PriceX',$condition);
