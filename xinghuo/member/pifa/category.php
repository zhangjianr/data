...
if(isset($_GET["action"])){
	if($_GET["action"]=="del"){
		$r = $DB->GetRs("pifa_category","count(*) as num","where Users_ID='".$_SESSION["Users_ID"]."' and Category_ParentID=".$_GET["CategoryID"]);
		if($r["num"]>0){
			echo '<script language="javascript">alert("该栏分类下有子分类,请勿删除");window.location="'.$_SERVER['HTTP_REFERER'].'";</script>';
		}else{
			$r = $DB->GetRs("pifa_products","count(*) as num","where Users_ID='".$_SESSION["Users_ID"]."' and Products_Category=".$_GET["CategoryID"]);
			if($r["num"]>0){
				echo '<script language="javascript">alert("该分类下有商品,请勿删除");window.location="'.$_SERVER['HTTP_REFERER'].'";</script>';
			}else{
				$Flag=$DB->Del("pifa_category","Users_ID='".$_SESSION["Users_ID"]."' and Category_ID=".$_GET["CategoryID"]);
				if($Flag){
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div id="products" class="r_con_wrap"> 
      ...
      <script language="javascript">$(document).ready(pifa_obj.products_category_init);</script>
      <div class="category">
