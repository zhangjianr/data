<?php
	
	require_once($_SERVER["DOCUMENT_ROOT"].'/Framework/Conn.php');
	//$_SESSION['user_type'] = 'employee';

	require_once('./employee/right.php');
	require_once('./employee/right_all.php');
	
	$path = $_SERVER["REQUEST_URI"];
	$path1 = explode('?',$path);

	$my_path = str_replace('member','',trim($path1[0],'/'));
	$arr_my_path = explode('/',$path1[0]);
	if(!in_array($arr_my_path[(count($arr_my_path)-1)],array('login.php','findpwd.php'))){
		if(empty($_SESSION["Users_ID"])){
			header("location:/member/login.php");
		}else{
			if(!empty($_SESSION['Users_Account']) && isset($_SESSION['user_type'])){
				if(empty($_SESSION['employee_id'])){
					header("location:/member/login.php");
				}
			}
		}
	}
	
	//过滤提交
	$_GPC = array();
	$_GPC = array_merge($_GET, $_POST, $_COOKIE);
	$_GPC = ihtmlspecialchars($_GPC);
	
	if(!empty($my_path)){
		if(count(array_filter($arr_my_path)) == 3){
			if(isset($_SESSION['employee_id'])){
				require_once('employee/vertify.php');
				check_right($path,$file,$file_all);
			}
		}
		//echo $my_path;
		require_once('.'.$my_path);
	}else{
		require_once('index.php');
	}
	
	function ihtmlspecialchars($value) {
		if (empty($value)) {
			return $value;
		} else {
			return is_array($value) ? array_map('ihtmlspecialchars', $value) : htmlspecialchars(trim($value));
		}
	}
?>