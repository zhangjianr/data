...
if($_POST){
	...
	}elseif($_POST['Dis_Agent_Type'] == 3){
		$Agent_Rate = '';
	}elseif($_POST['Dis_Agent_Type'] == 4){
		$Agent_Rate = json_encode($_POST['Agent_Rate'],JSON_UNESCAPED_UNICODE);
	}
	...
	$shop_config->Dis_Level = json_encode($_POST['Dis_Level'],JSON_UNESCAPED_UNICODE);
      ...
      <form id="products_form" class="r_con_form" method="post" action="distribute_config.php">
        <div class="rows">
          ...
          <span class="input">
           <select name="Type" id="type">
			 ...
			 <option value="5"<?php echo $rsConfig["Distribute_Type"]==5 ? ' selected' : '';?>>购买星火自动成为分销商</option>
           </select>
          </span>
          <div class="clear"></div>
        </div>
        ...
        <div class="rows">
         ...
         <div class="rows" >
            ...
            <span class="input">
                <?php
					$dis_type_list = array('关闭','合伙人','地区代理','渠道推广',"代理+渠道");
				?>
                <?php
				//var_dump($dis_type_list);
				foreach($dis_type_list as $key=>$agent_name):?>
               <input type="radio" name="Dis_Agent_Type" value="<?=$key?>" <?=$key==$rsConfig["Dis_Agent_Type"]?'checked':''?> <?php if($key==1){ ?> style="display:none" <?php } ?> /><label <?php if($key==1){ ?> style="display:none" <?php } ?> for="c_0"><?=$agent_name?></label>&nbsp;&nbsp;
                <?php endforeach;?>
            </span>
        </div>
        ...
        <?php if($rsConfig['Dis_Agent_Type'] != 0):?>
                ...
                <?php elseif($rsConfig['Dis_Agent_Type'] == 2 || $rsConfig['Dis_Agent_Type'] == 4): ?>
                  <?php
				 ...
				 县%<input type="text" name="Agent_Rate[County]" value="<?= $Agent_Rate_list['County']?>" class="form_input" size="3" maxlength="10" notnull />
	   			<?php endif; ?>
...
$Count_Level=count($Dis_Levels);
?>
<div class="rows">
           <label>分销商分配比例</label>
           <span class="input">
            <div class="dis_add tips" onclick="add_one_form()"><img src="/static/member/images/ico/add.gif" align="absmiddle" /> 增加层次<font style="color:#F00">(统一设置默认的渠道分配层数和比例)</font></div>
			<div class="blank9"></div>
			
							<table class="level_table" border="0" cellpadding="5" cellspacing="0" style="width:300px">
								<thead>
									<tr>
										<td width="10%">层次</td>
										<td width="15%">比例</td>
								...
								<?php if (!empty($Dis_Levels)): 
								$count=count($Dis_Levels);
								?>
										<?php foreach ($Dis_Levels AS $key=>$val) { ?>

											<tr  fieldtype="text" class="level_<?=$key?>">
												<td><?= $key+1 ?></td>
												<td>
													<input class="form_input" value="<?= $val['Rate'] ?>" name="Dis_Level[<?= $key ?>][Rate]" type="text">%
												</td>
												<td>
												<?php if ($key+1==$count):?>
												<a class="level_del" href="javascript:void(0);" level_id="<?=$key?>">
									...
									<?php else: ?>
										<tr  fieldtype="text">
											<td>1</td>
											<td>
												<input class="form_input" value="" name="Dis_Level[0][Rate]" type="text">%
											</td>
										</tr>
									<?php endif;?>
								</tbody>
...
<script>
			...
			function add_one_form(){
				...
				var tpl = '<tr class="level_'+i+'">\
								<td>'+(i+1)+'</td>\
								<td>\
									<input class="form_input" value="" name="Dis_Level['+i+'][Rate]" type="text">\
								</td>\
								<td>\
