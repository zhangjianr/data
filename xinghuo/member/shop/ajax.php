...
if($action == 'mod_bankcard'){
   ...
   echo json_encode($response,JSON_UNESCAPED_UNICODE);
	...
	//print_r($area_json);exit;
	$area_array = json_decode($area_json,TRUE);
	$county_list = array();
	foreach($area_array as $key=>$val){
		$tmp_key = explode(",",$key);
		if(count($tmp_key) == 3){
			$county_list[$area_array[$tmp_key[0].",".$tmp_key[1]][$tmp_key[2]]] = $val;
		}
	}
	...
	$agent_county = $agent_areas->filter(function($agent_area){
	...
	$agent_county_dropdown = get_dropdown_list($agent_county,'area_id');
	...
	foreach($agent_county_dropdown as $area_id=>$agent_area){
    	if($agent_area['Account_ID'] == $account_id ){
    		$county_checked[] = $area_id."_".$agent_area['area_name'];
    	}else{
    		$county_disabled[] = $area_id;
    	}
    }
	$county_data_list['disable'] = $county_disabled;
	$county_data_list['checked'] = !empty($county_checked) ? json_encode($county_checked) : "0";
	...
	$County = !empty($_POST['County'])?$_POST['County']:array();
...
}else if($action == 'get_dis_agent_one'){
	
	$Account_ID = $_GET['account_id'];
	//获取独立合伙人设置信息
	$agent_one = Dis_Agent_One::where(array('Users_ID'=>$UsersID,'Account_ID'=>$Account_ID))->first();
	if(!empty($agent_one)){
		$response = array('status'=>1,'content'=>$agent_one,'msg'=>'获取信息成功');
	}else{
		$response = array('status'=>0,'content'=>$agent_one,'msg'=>'获取信息失败');
	}
	echo json_encode($response,JSON_UNESCAPED_UNICODE);

}else if($action == 'save_dis_agent_one'){
	
	//构造数据
	$Account_ID = intval($_POST['account_id']);
	$Agent_Rate = empty($_POST['Agent_Rate']) ? "0" : trim($_POST['Agent_Rate']);
	$user = Dis_Agent_One::Multiwhere(array('Users_ID'=>$UsersID,'Account_ID'=>$Account_ID))->first();
	$dis_account = Dis_Account::find($Account_ID);
	if(empty($user) && $Agent_Rate > 0){
		$res = new Dis_Agent_One;
		$res->Users_ID = $UsersID;
		$res->Account_ID = $Account_ID;
		$res->Agent_Rate = $Agent_Rate;
		$res->create_at = time();
		$res->status = 1;
		$res->save();
		$dis_account->Enable_Agent = 3;
		$dis_account->save();
		if($res != false && $dis_account != false){
			$response = array('status'=>1);
		}
	}else{
		if($Agent_Rate == 0){
			$user->delete();
			$response = array('status'=>1);
		}else if($Agent_Rate > 0){
			$user->Agent_Rate = $Agent_Rate;
			$user->save();
			$dis_account->Enable_Agent = 3;
			$dis_account->save();
			$response = array('status'=>1);
...
}else if($action == 'get_channel_type'){
	
	$Account_ID = $_GET['account_id'];
	//获取渠道推广设置信息
	$channel_type = Dis_Account::where(array('Users_ID'=>$UsersID,'Account_ID'=>$Account_ID))->first();
	if(!empty($channel_type)){
		$response = array('status'=>1,'content'=>$channel_type,'msg'=>'获取信息成功');
	}else{
		$response = array('status'=>0,'content'=>$channel_type,'msg'=>'获取信息失败');
	}
	echo json_encode($response,JSON_UNESCAPED_UNICODE);

}else if($action == 'save_channel_type'){
	
	//构造数据
	$Account_ID = intval($_POST['account_id']);
	$type_id = empty($_POST['type_id']) ? "0" : trim($_POST['type_id']);
	
	if($type_id!=0){
		$dis_account = Dis_Account::find($Account_ID)->update(array('Channel_Type'=>$type_id,'Enable_Agent'=>3));;
		$response = array('status'=>1);
	}else{
		$dis_account = Dis_Account::find($Account_ID)->update(array('Channel_Type'=>0,'Enable_Agent'=>0));;
		$response = array('status'=>1);
	}
