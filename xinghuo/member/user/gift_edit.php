...
if($_POST)
{
	$Data=array(
		...
		"Gift_FreeShipping"=>isset($_POST['FreeShipping']) ? $_POST['FreeShipping'] : 0
	);
    ...
    <div id="gift" class="r_con_wrap">
      ...
      <form id="gift_edit_form" class="r_con_form" method="post" action="gift_edit.php">
        ...
        <div class="rows">
          <label>需要物流</label>
          <span class="input price">
          <select name="Shipping" onChange="if(this.value==1){document.getElementById('freeshipping').style.display='';}else{document.getElementById('freeshipping').style.display='none';}">
		...
		<div class="rows" id="freeshipping" style="display:<?php echo $rsGift['Gift_Shipping']==0?'none':'block' ?>">
          <label>是否免运费</label>
          <span class="input">
			<input type="checkbox" name="FreeShipping" value="1"<?php echo $rsGift['Gift_FreeShipping']==1 ? ' checked' : '' ?> /> 免运费
          </span>
          <div class="clear"></div>
        </div>
        <div class="rows">
