...
if(isset($_GET["search"])){
	if($_GET["search"]==1){
		if(!empty($_GET["Keyword"])){
			$condition .= " and (User_Mobile like '%".$_GET["Keyword"]."%' or User_Name like '%".$_GET["Keyword"]."%')";
		}
	...
	if($action=="money_mod")
	{
		...
		if($_POST['Value']<0){
			...
			$Data=array(
				...
				'Amount'=>$_POST['Value'],
				'Total'=>$rsUser['User_Money']+$_POST['Value'],
				'Note'=>$_SESSION["Users_Account"]." 线下充值 +".$_POST['Value'],
				'CreateTime'=>time()			
			);
      ...
      <table width="100%" align="center" border="0" cellpadding="5" cellspacing="0" class="r_con_table">
		...
		while($rsUser=$DB->fetch_assoc()){?>
          <tr UserID="<?php echo $rsUser['User_ID'] ?>">
            ...
            <td nowrap="nowrap"><?php echo empty($rsUser['User_From'])?'注册':'微信' ?></td>
            ...
            <td nowrap="nowrap" class="last"><?php echo empty($rsUser['User_From'])?'<a href="#modpass"><img src="/static/member/images/ico/mod.gif" align="absmiddle" /></a> ':'' ?><a href="user_view.php?UserID=<?php echo $rsUser['User_ID'] ?>"><img src="/static/member/images/ico/view.gif" align="absmiddle" /></a> <a href="user_list.php?action=del&UserID=<?php echo $rsUser['User_ID'] ?>" onClick="if(!confirm('删除后不可恢复，继续吗？')){return false};"><img src="/static/member/images/ico/del.gif" align="absmiddle" /></a></td>
          </tr>
          <?php $i++;
		  }?>
        </tbody>
