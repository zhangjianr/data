...
while($rsPmenu=$DB->fetch_assoc()){
	echo '<option value="'.$rsPmenu["Menu_ID"].'"'.($rsPmenu["Menu_ID"]==$rsMenu["Menu_ParentID"]?" selected":"").'>'.$rsPmenu["Menu_Name"].'</option>';
            ...
            <div class="opt_item">
              <label>消息类型：</label>
              <span class="input">
              <select name="MsgType" onChange="document.getElementById('menu0').style.display='none';document.getElementById('menu1').style.display='none';document.getElementById('menu2').style.display='none';document.getElementById('menu'+this.value).style.display='';">
            ...
            <div class="opt_item" id="menu1"<?php echo $rsMenu["Menu_MsgType"]==1?'':' style="display:none;"'; ?>>
              ...
              <select name='MaterialID'>
				...
				while($rsMaterial=$DB->fetch_assoc()){
					...
					echo '<option value="'.$rsMaterial['Material_ID'].'"'.($rsMenu["Menu_MaterialID"]==$rsMaterial['Material_ID']?" selected":"").'>'.$json['Title'].'</option>';
				}?>
				...
				while($rsMaterial=$DB->fetch_assoc()){
					...
					echo '<option value="'.$rsMaterial['Material_ID'].'"'.($rsMenu["Menu_MaterialID"]==$rsMaterial['Material_ID']?" selected":"").'>'.($rsMaterial['Material_Type']?'【多图文】':'【单图文】').$json['Title'].'</option>';
				}?>
              </select>
            ...
            <div class="opt_item" id="menu2"<?php echo $rsMenu["Menu_MsgType"]==2?'':' style="display:none;"'; ?>>
              <label>链接网址：</label>
              <span class="input">
              <input type="text" name="Url" value="<?php echo $rsMenu["Menu_Url"] ?>" class="form_input" size="35" maxlength="200" />
              </span>
              <div class="clear"></div>
            </div>
            <div class="opt_item">
