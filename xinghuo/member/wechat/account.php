...
require_once($_SERVER["DOCUMENT_ROOT"].'/include/library/update.class.php');
/*升级检测开始*/
//	if(is_file($_SERVER["DOCUMENT_ROOT"]."/version.txt")){
//		$ver = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/version.txt");
//	}else{
//		$ver = 0;
//	}
//	$updateUrl = "http://spark.weiyuntop.com/update/version.php?ver=".$ver;
//	//初始化
//	$ch = curl_init();
//	//设置选项，包括URL
...
//	curl_close($ch);
//	//打印获得的数据
//	$serve = json_decode($output, TRUE);
//	$updateMsg = $serve["msg"];
//	var_dump($serve);
/*升级检测结束*/

if(!empty($_POST['op']) && $_POST['op'] == "updateSystem"){
	$update = new update($ver);
	$resUpdate = $update->start();
	exit($resUpdate);
}
...
if(empty($_SESSION["Users_Account"]))
{
	...
	//今日已付款订单
	$today_payed_order_num = $order->statistics($Users_ID,'num',$today,$now,4);
	//今日销售额
	$today_order_sales = $order->statistics($Users_ID,'sales',$today,$now);
	$today_payed_order_sales = $order->statistics($Users_ID,'sales',$today,$now,4);
	//本月销售额
	$month_order_sales = $order->statistics($Users_ID,'sales',$month_start,$month_end);
	$month_payed_order_sales = $order->statistics($Users_ID,'sales',$month_start,$month_end,4);
	//今日支出佣金
...
<html lang="en">
	...
	$(function(){
		$("#updateSystem").click(function(){
			var updateMsg = layer.msg('正在更新升级，请勿关闭浏览器或刷新页面！！！', {icon: 16,shade: 0.3,time: 0});
			$.post(
				"",
				{op : "updateSystem"},
				function(data){
    ...
    <div id="wrap"> 
      ...
      <div id="welcome"  >
		  <p style="color:#F56059">
			  <?= $updateMsg ?>
			  <a href="javascript:void(0)" id="updateSystem" class="btn btn-sm btn-info">更新系统</a>
		  </p>
      </div>
	  <?php endif; ?>
      <!-- tip end --> 
      <!-- statistics begin -->
		...
		<dd class="statis-item " id="month_sales">
          <p> <span class="statis-name pull-left">今日已付款销售额</span> <span class="statis-num pull-right">
            <?=$today_payed_order_sales?>
		...
		<dd class="statis-item " id="month_sales">
          <p> <span class="statis-name pull-left">本月已付款销售额</span> <span class="statis-num pull-right">
            <?=$month_payed_order_sales?>
            <img  src="/static/member/images/accout_back/coin_and_paper.png"/></span> </p>
        </dd>
