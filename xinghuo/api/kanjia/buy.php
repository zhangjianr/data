...
$shipping_list_temp = $rsPay['Shipping'] ? json_decode($rsPay['Shipping'],true) : array(); 
$shipping_list = array();

foreach($shipping_list_temp as $key=>$item){
	$item['Base_Price'] = empty($item['Base_Price']) ? 0.00 : number_format($item['Base_Price'],2);
	$shipping_list[] = $item;
}
...
$shipping_price = empty($shipping_list[0]['Price']) ? 0 : $shipping_list[0]['Price']; 
