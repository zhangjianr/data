	...
	<div class="index-h">
	    ...
	    <?php
			...
			foreach($Category_List as $rscategory){
				...
				$i++;
				$DB->get("shop_category","Category_ID","where Users_ID='".$UsersID."' and Category_ParentID=".$rscategory["Category_ID"]." order by Category_Index asc");
				$cates = array();
				while($r=$DB->fetch_assoc()){
					$cates[] = $r["Category_ID"];
				}
				...
				if(empty($cates)){
					$condition .= " and Products_Category=".$rscategory["Category_ID"]." order by Products_ID desc";
				}else{
					$cates[] = $rscategory["Category_ID"];
					$condition .= " and Products_Category in(".implode(",",$cates).") order by Products_ID desc";
				}
