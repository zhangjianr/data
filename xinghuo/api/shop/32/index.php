...
function get_products_bycate($db,$catid){
	...
	$new_products = array();
	$rsNewProducts = $db->get("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_PriceY,Products_Count,Products_Sales","where Products_SoldOut=0 and Products_IsRecommend=1 and Products_Category in(".implode(",",$CategoryList).") order by Products_CreateTime asc limit 0,4");
	$new_products = handle_product_list($db->toArray($rsNewProducts));
	return $new_products;
}
?>
