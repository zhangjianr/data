...
if(!empty($_POST)){
	if(empty($_POST['cate_id'])){
		...
		$rsNewProducts = $DB->get("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_Sales","where Users_ID='".$UsersID."' and Products_IsNew=1 and Products_SoldOut=0 order by Products_CreateTime asc limit $limitpage,$num");
		...
		$new_products = array();
		$counts = $DB->GetRs("shop_products","count(Products_ID) as count","where Products_SoldOut=0 and Products_Category in(".implode(",",$CategoryList).")");
		$num = 6;//每页记录数
		if($counts['count'] <= $num){
			$show_load_btn = 0;
		}
		...
		$rsNewProducts = $DB->get("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_Sales","where Products_SoldOut=0 and Products_Category in(".implode(",",$CategoryList).") order by Products_CreateTime asc limit $limitpage,$num");
		$new_products = handle_product_list($DB->toArray($rsNewProducts));
		foreach($new_products as $k => $item){
			$new_products[$k]['link'] = $shop_url.'products/'.$item['Products_ID'].'/';
		}
   ...
   <div class="shop_skin_index_list banner" rel="edit-t01">
	...
	<div class="box">
		<div class="shop_skin_index_list1">
			<div class="img"><img src="/static/api/shop/skin/31/i0.png" /></div>
