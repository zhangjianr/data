...
if(empty($action)){
	...
	//积分使用判断
	if(isset($_POST['User_Integral'])){
		$data = array("User_Integral"=>$_POST["User_Integral"]);
		$condition = "where Users_ID='".$UsersID."' and User_ID='".$_SESSION[$UsersID."User_ID"]."'";
		$DB->set('user',$data,$condition);
	}
	...
	if($_SESSION[$UsersID.'User_ID'] == $owner['id']&&$ds_account['invite_id'] == '0'){
		$is_distribute = FALSE;
	}
	...
	if($is_distribute){
		foreach($CartList as $ProductID=>$product_list){
			foreach($product_list as $key=>$item){
				if($item['OwnerID']>0){
					add_distribute_record($UsersID,$DB,$item['OwnerID'],$item['ProductsPriceX'],$ProductID,$item['Qty'],$neworderid);
				}
			}
		}
	}
