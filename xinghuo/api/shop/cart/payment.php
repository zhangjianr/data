  ...
  <div id="payment">
    <div class="i-ture">
      ...
      <div class="info"> 订 单 号：<?php echo date("Ymd",$rsOrder["Order_CreateTime"]).$rsOrder["Order_ID"] ?><br />
        订单总价：<span class="fc_red" id="Order_TotalPrice">￥<?php echo $rsOrder["Order_TotalPrice"]?><br/></span>
...
if(!empty($rsPay["Payment_OfflineEnabled"])){?>
             <li>
             <a href="/api/<?=$UsersID?>/shop/cart/complete_pay/huodao/<?=$OrderID?>/" class="btn btn-warning  btn-pay" id="out" data-value="线下支付"><img  src="/static/api/shop/skin/default/huodao.png" width="16px" height="16px"/>&nbsp;货到付款</a>
            </li>
    <?php }?>        

             <li>
             <a href="/api/<?=$UsersID?>/shop/cart/complete_pay/money/<?=$OrderID?>/" class="btn btn-warning  btn-pay" id="money" data-value="余额支付"><img  src="/static/api/shop/skin/default/money.jpg"  width="16px" height="16px"/>&nbsp;余额支付</a>
            </li>	
            </ul>
