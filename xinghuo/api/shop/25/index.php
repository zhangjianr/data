...
if(!empty($_POST)){
	if(empty($_POST['cate_id'])){
		//获取新品
		$counts = $DB->GetRs("shop_products","count(Products_ID) as count","where Users_ID='".$UsersID."' and Products_IsNew=1 and Products_SoldOut=0");
		$num = 8;//每页记录数
		$p = !empty($_POST['p'])?intval(trim($_POST['p'])):1;
		$total = $counts['count'];//数据记录总数
		$totalpage = ceil($total/$num);//总计页数
		$limitpage = ($p-1)*$num;//每次查询取记录
		$rsNewProducts = $DB->get("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_Count,Products_Sales","where Users_ID='".$UsersID."' and Products_IsNew=1 and Products_SoldOut=0 order by Products_CreateTime asc limit $limitpage,$num");
		...
		//获取推荐商品
		$counts = $DB->GetRs("shop_products","count(Products_ID) as count","where Users_ID='".$UsersID."' and Products_IsRecommend=1 and Products_SoldOut=0");
		$num = 6;//每页记录数
		$p = !empty($_POST['p'])?intval(trim($_POST['p'])):1;
		$total = $counts['count'];//数据记录总数
		$totalpage = ceil($total/$num);//总计页数
		$limitpage = ($p-1)*$num;//每次查询取记录
		$rsRecommendProducts = $DB->get("shop_products","Products_Name,Products_ID,Products_JSON,Products_PriceX,Products_Count,Products_Sales","where Users_ID='".$UsersID."' and Products_IsRecommend=1 and Products_SoldOut=0 order by Products_CreateTime asc limit $limitpage,$num");
...
<div id="shop_page_contents">
 ...
 <div id="shop_skin_index">
   <div class="shop_skin_index_list banner" rel="edit-t01">
    ...
    <?php require_once('index_info.php');?>
    <div class="clear"></div>
 </div>
