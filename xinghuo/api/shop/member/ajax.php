...
if($action=="address"){
	...
	echo json_encode($response,JSON_UNESCAPED_UNICODE);
	    ...
	    $Order_ID = $_POST['Order_ID'];
		$get_status=$DB->GetRs("user_order","Order_Status","WHERE Users_ID='".$UsersID."' and Order_ID=".$Order_ID);
		if($get_status['Order_Status'] ==4){
			$response = array(
				"status"=>2,
				"msg"=>'确认收货失败'
			);
		...
		if($Flag)
		{
			$response = array(
				"status"=>1,
				"url"=>$base_url.'api/'.$UsersID.'/shop/member/backup/status/3/'
			);
