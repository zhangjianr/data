...
while($rsOrder=$DB->fetch_assoc()){
    ...
    <div class="item">
	  ...
	  <h1 style='height:30px;line-height:30px;'> 物流单号：<a href="<?php $shipping = json_decode($rsOrder['Order_Shipping'],true) ;echo 'http://m.kuaidi100.com/index_all.html?type='.$shipping['Express'].'&postid='.$rsOrder["Order_ShippingID"].'&callbackurl='.'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']; ?>"><?php echo $shipping['Express'].'-'.$rsOrder["Order_ShippingID"] ?></a><a href="<?php echo 'http://m.kuaidi100.com/index_all.html?type='.$shipping['Express'].'&postid='.$rsOrder["Order_ShippingID"].'&callbackurl='.'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']; ?>"><span style='background-color:#DE2337;padding:3px 10px;border-radius:5px;color:#FFF;margin-left:15px;'>快递跟踪</span></a> </h1>
	  <?php }?>
<?php
	if(isset($CartList)){
