...
if(isset($_SESSION[$UsersID."User_ID"])){
	...
	if($action == 'change_shipping_method'){
		...
		$total_info = get_gift_shipping_fee($UsersID,$rsConfig,$Shipping_ID,$City_Code);
		...
		$item = $DB->GetRs("user_gift_orders","*","where Users_ID='".$UsersID."' and User_ID=".$_SESSION[$UsersID."User_ID"]." and Gift_ID=".$_POST['GiftID']." and Orders_Status<>4");
		if($item){
			$Data = array(
				"status"=>0,
				"msg"=>"您已兑换过此礼品，请勿重复兑换",
		...
		if($rsUser['User_Integral']<$rsGift['Gift_Integral']){
			$Data = array(
				"status"=>0,
				"msg"=>"您的积分不足",
				"url"=>'/api/'.$UsersID.'/user/gift/'
		...
		if($rsGift['Gift_Shipping']==0){
			...
			$Data=array(
				...
				"Orders_Integral"=>$rsGift['Gift_Integral']
			);
			$Flag=$Flag&&$DB->Add('user_gift_orders',$Data);
			if($Flag){
				$orderid=$DB->insert_id();
				$DB->Set("user_gift_orders","Orders_Code='".rand(1000000,9999999).$orderid."'","where Orders_ID=".$orderid);
				$sms_mess = '您已成功兑换礼品，兑换号：'.rand(1000000,9999999).$orderid.'请及时换取！';
				send_sms($_POST["Mobile"], $sms_mess, $rsGift["Users_ID"]);
				$url = '/api/'.$UsersID.'/user/gift/my/';
			}
		}else{
			//更新用户表用户不可用积分和积分			
			$Flag=$Flag&&$DB->Set("user","User_UseLessIntegral=User_UseLessIntegral+".$rsGift['Gift_Integral'].",User_Integral=User_Integral-".$rsGift['Gift_Integral'],"where Users_ID='".$UsersID."' and User_ID=".$UserID);
			...
			if($_POST["total_price"]>0 && $rsGift['Gift_FreeShipping']==0){
			...
			if($Flag){
				if($_POST["total_price"]>0 && $rsGift['Gift_FreeShipping']==0){
					$url = '/api/'.$UsersID.'/user/gift/payment/'.$DB->insert_id()."/";
				}else{
					$url = '/api/'.$UsersID.'/user/gift/my/';
				}
			}
		...
		$rsOrder=$DB->GetRs("user_gift_orders","*","where Users_ID='".$UsersID."' and Orders_ID='".$OrderID."'");
		...
		if($_POST['PaymentMethod']=="线下支付"){
			...
			$Status=1;
			$Flag=$Flag&&$DB->Set("user_gift_orders",$Data,"where Users_ID='".$UsersID."' and User_ID='".$_SESSION[$UsersID."User_ID"]."' and Orders_ID=".$OrderID);
			$url="/api/".$UsersID."/user/gift/my/";
			if($Flag){
				$Data=array(
					"status"=>1,
			...
			if($rsOrder["Orders_Status"] != 0){
				...
				$Data=array(
					...
					'Note'=>"积分换礼运费支出 -".$rsOrder["Orders_TotalPrice"]." (订单号:".$OrderID.")",
					'CreateTime'=>time()		
				);
			...
			$url="/api/".$UsersID."/user/gift/pay/".$OrderID."/".$PaymentMethod[$_POST['PaymentMethod']]."/";
		...
		$rsOrder=$DB->GetRs("user_gift_orders","*","where Users_ID='".$UsersID."' and Orders_ID='".$ordersid."'");
		if($rsOrder["Orders_Status"]==0 && $rsOrder["Orders_IsShipping"]==1){
			$Flag=$Flag&&$DB->Set("user_gift_orders","Orders_Status=4","where Users_ID='".$UsersID."' and User_ID='".$_SESSION[$UsersID."User_ID"]."' and Orders_ID=".$ordersid);
			$Flag=$Flag&&$DB->Set("user","User_UseLessIntegral=User_UseLessIntegral-".$rsOrder['Orders_Integral'].",User_Integral=User_Integral+".$rsOrder['Orders_Integral'],"where Users_ID='".$UsersID."' and User_ID=".$UserID);
			$Flag=$Flag&&$DB->Set("user_gift","Gift_Qty=Gift_Qty+1","where Users_ID='".$UsersID."' and Gift_ID=".$rsOrder['Gift_ID']);
			if($Flag){
				$Data = array(
					"status"=>1,
					"url"=>"/api/".$UsersID."/user/gift/my/"
		...
		if($rsOrder["Orders_Status"]==2 && $rsOrder["Orders_IsShipping"]==1){
			$Flag=$Flag&&$DB->Set("user_gift_orders","Orders_Status=3,Orders_FinishTime=".time(),"where Users_ID='".$UsersID."' and User_ID='".$_SESSION[$UsersID."User_ID"]."' and Orders_ID=".$ordersid);
			$Flag=$Flag&&$DB->Set("user","User_UseLessIntegral=User_UseLessIntegral-".$rsOrder['Orders_Integral'],"where Users_ID='".$UsersID."' and User_ID=".$UserID);
			$Data=array(
				'Record_Integral'=>-$rsOrder["Orders_Integral"],
