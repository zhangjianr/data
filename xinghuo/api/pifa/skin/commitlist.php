<?php require_once('header.php'); ?>
  ...
  <div class="commit_title"><a href="/api/<?php echo $UsersID;?>/pifa/product/<?php echo $ProductsID;?>/"><?php echo $rsProducts["Products_Name"];?></a></div>
  <div class="commit_list">
   <?php
	  $commit = $DB->GetRs("user_order_commit","count(*) as num, sum(Score) as score","where Status=1 and MID='pifa' and Product_ID=".$ProductsID);
	  $num = $commit["num"];
	  $score = $commit["score"];
   ...
   <?php
      $DB->getPage("user_order_commit","*","where Status=1 and MID='pifa' and Product_ID=".$ProductsID." order by CreateTime DESC",$pageSize=20);
  ...
  <?php $DB->showWechatPage('/api/'.$UsersID.'/pifa/commit/'.$ProductsID.'/'); ?>
