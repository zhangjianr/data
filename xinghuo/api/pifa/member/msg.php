...
if(empty($_SESSION[$UsersID."User_ID"]))
{
	$_SESSION[$UsersID."HTTP_REFERER"]="/api/".$UsersID."/pifa/distribute/";
...
$(document).ready(pifa_obj.distribute_init);
</script>
</head>

<body>
<header class="bar bar-nav"> <a  href="javascript:history.back()"class="icon icon-2x icon-chevron-left grey pull-left"></a> <a  href="<?=$base_url?>api/<?=$UsersID?>/pifa/distribute/" class="icon icon-2x icon-sitemap grey pull-right"></a>
...
<div id="page_contents">
	...
	<div id="message-list">
		...
		<div class="item">
			<h1><i class="icon icon-comment-alt green"></i>&nbsp;&nbsp;
			...
			<p> <span style="float:left">
				<?=date("Y-m-d H:i:s",$message['Message_CreateTime'])?>
				</span> <span style="float:right"><a  class="remove-msg" href="javascript:void(0)"><i class="red icon icon-remove"></i></a></span> </p>
		</div>
		<?php endforeach;?>
	</div>
</div>
