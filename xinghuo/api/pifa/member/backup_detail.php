...
<script language="javascript">$(document).ready(pifa_obj.backup_init);</script>
</head>
...
<div id="shop_page_contents">
	...
	<ul id="member_nav">
		...
		<li class=""><a href="/api/<?php echo $UsersID ?>/pifa/member/address/">地址簿</a></li>
		<?php }?>
	</ul>
	<div id="order_detail">
		<div class="item">
					...
					<tr>
						...
						<td>&yen;
							...
							*
							<?=$product['back_num']?>
							=&yen;
							<?=$product['Products_Price']*$product['back_num']?></td>
					</tr>
					<?php endforeach;?>
				</tbody>
			</table>
		...
		<div class="item">
			...
			<dl>
				<dd>快递公司:
					<?=$rsBackup["Back_Shipping"]?>
				</dd>
				<dd>快递单号:
					<?=$rsBackup["Back_ShippingID"]?>
				</dd>
			</dl>
		</div>
		<?php endif;?>
		<?php if($rsBackup["Back_Status"] == 1):?>
		<form action="<?=$base_url?>api/<?=$UsersID?>/pifa/member/" method="post" name="backup_shipping_form" id="backup_shipping_form">
