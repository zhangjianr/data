...
if(!empty($_GET["action"])&&$_GET["action"]=="Del"){
	...
	mysql_query("delete from `update` where id='".$ID."'");	
...
<div id="iframe_page">
  <div class="iframe_content">
    ...
    <div class="r_con_wrap">
      ...
      <div class="b10"></div>
      <a href="add.php" style="display:block; width:80px; height:26px; line-height:26px; text-align:center; border-radius:5px; background:#3AA0EB; color:#FFF; font-size:12px;">添加日志</a>
      <div class="b10"></div>
      <table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        ...
        <?php 
			$lists = array();
			$DB->getPage("update","*"," order by id desc",10);
			while($r=$DB->fetch_assoc()){
				$lists[] = $r;
			}
			foreach($lists as $t){
				$category = $DB->GetRs("update_category","catname","where id=".$t["catid"]);
		?>
          <tr>
            <td nowrap="nowrap"><?php echo $t["id"] ?></td>
            <td nowrap="nowrap"><?php echo empty($category["catname"]) ? '' : $category["catname"]; ?></td>
            <td style="text-align:left; padding-left:15px;"><?php echo $t["title"];?></td>
