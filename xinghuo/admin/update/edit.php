...
$item=$DB->GetRs("update","*","where id=".$id);
if(empty($item)){
	echo '<script language="javascript">alert("此日志已经不存在！");window.location="javascript:history.back()";</script>';
	exit();
}
if($_POST){
	...
	$flag=$DB->Set("update",$Data,"where id=".$id);
	if($flag){
  ...
  <div class="iframe_content">
    ...
    <div class="r_con_wrap">
		<form class="r_con_form" method="post" action="?ID=<?php echo $id;?>">
                 ...
                 <?php
				   ...
				   foreach($lists as $r){
						...
						while($t=$DB->fetch_assoc()){
							echo '<option value="'.$t["id"].'"'.($item["catid"]==$t["id"] ? ' selected' : '').'>&nbsp;└&nbsp;'.$t["catname"].'</option>';
						}
						echo '</optgroup>';
				   }
			       ?>
            ...
            <div class="rows">
                <label>标题</label>
                <span class="input"><input type="text" name="title" value="<?php echo $item["title"];?>" size="30" class="form_input" /></span>
                <div class="clear"></div>
            </div>
            ...
            <div class="rows">
                <label>详细内容</label>
                <span class="input">
                    <textarea name="content" style="width:100%;height:400px;visibility:hidden;"><?php echo $item["content1"];?></textarea>
                </span>
                <div class="clear"></div>
            </div>
