...
if(isset($_GET["action"])){
	if($_GET["action"]=='search'){
		if(!empty($_GET["kw"])){
			$fields = $_GET["fields"];
			$kw = $_GET["kw"];
			if($_GET["fields"]=='company'){
				$ids = "'0'";
				$DB->get("users","Users_ID","where Users_Company LIKE '".$kw."'");
  ...
  <div class="iframe_content">
    ...
    <div class="r_con_wrap">
      <form class="search" id="search_form" method="get" action="?">
        ...
        <select name="fields">
         <option value="company"<?php echo $fields=='company' ? ' selected' : '';?>>商家名称</option>
      ...
      <table border="0" cellpadding="5" cellspacing="0" class="r_con_table">
        ...
        <?php
			...
			foreach($lists as $t){
				$company = $DB->GetRs("users","Users_Company","where Users_ID='".$r["Users_ID"]."'");
				$t["Company"] = $company["Users_Company"];
		?>
          <tr>
